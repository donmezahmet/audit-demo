...
const DepartmentActionsPage: React.FC = () => {
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    const stored = localStorage.getItem('department-actions-column-order');
  ...
  // Resizable columns (removed impact, added description and riskLevel)
  ...
  const columnDefinitions: Record<string, { label: string; render: (action: any) => React.ReactNode; sortable?: boolean; sortKey?: string }> = {
    key: {
      label: 'Key',
      render: (action) => <span className="text-purple-600 font-mono text-sm">{action.key}</span>
    },
    summary: {
      label: 'Summary',
    ...
    description: {
      label: 'Description',
      render: (action) => <span className="text-gray-700 text-sm line-clamp-2">{action.description || '-'}</span>
    },
    status: {
      label: 'Status',
      sortable: true,
    ...
    audit: {
      ...
      sortable: true,
      sortKey: 'auditName',
      render: (action) => <span className="text-gray-600 text-sm">{action.auditName || '-'}</span>
    },
    dueDate: {
      label: 'Due Date',
      sortable: true,
      sortKey: 'dueDate',
      render: (action) => <span className="text-gray-600 text-sm">{action.dueDate ? formatDate(action.dueDate, 'PP') : '-'}</span>
    },
    riskLevel: {
      label: 'Risk Level',
      sortable: true,
    ...
    responsible: {
      label: 'Responsible',
      render: (action) => <span className="text-gray-600 text-sm">{action.responsibleEmail || '-'}</span>
    },
    actions: {
      label: 'Action Detail',
      render: (action) => (
  ...
  const paginatedActions = useMemo(() => {
    ...
    return processedActions.slice(startIndex, endIndex);
  }, [actions, currentPage, itemsPerPage, sortField, sortDirection, statusFilter, auditFilter, riskLevelFilter]);
  ...
  const statusChartData: ChartData<'pie'> = useMemo(() => {
    ...
    return {
      ...
      datasets: [{
        data,
        backgroundColor: labels.map(s => colors[s] || 'rgba(147, 51, 234, 0.8)'),
        borderColor: labels.map(s => (colors[s] || 'rgba(147, 51, 234, 0.8)').replace('0.8', '1')),
        borderWidth: 2,
      }],
    };
  }, [actions]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Department Actions</h1>
        ...
        >
          <div className="text-center">
            <p className="text-2xl font-bold text-red-600">
              {formatFinancialImpact(stats.moneyOpen)}
            </p>
            <p className="text-xs text-gray-600 mt-1">Open Impact</p>
        ...
        >
          <div className="text-center">
            <p className="text-2xl font-bold text-yellow-600">
              {formatFinancialImpact(stats.moneyOverdue)}
            </p>
            <p className="text-xs text-gray-600 mt-1">Overdue Impact</p>
            <p className="text-[10px] text-gray-400 mt-0.5">(2024+)</p>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card variant="elevated">
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
              <span className="text-gray-700 font-medium">Overdue Rate</span>
              <span className="text-2xl font-bold text-yellow-600">
                {stats.total > 0 ? Math.round((stats.overdue / stats.total) * 100) : 0}%
              </span>
            </div>
            <div className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
              <span className="text-gray-700 font-medium">Total Financial Impact</span>
      ...
      <Card variant="elevated">
        <CardHeader 
          ...
          action={
            <Button 
              variant="primary" 
              size="sm"
              onClick={() => exportFindingActions({ auditYear: scorecardFilter, role: 'department' })}
        ...
        {isLoading ? (
          <div className="h-64 flex items-center justify-center">
            <Loading size="xl" text="Loading department actions..." />
          </div>
