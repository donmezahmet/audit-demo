...
const MyActionsPage: React.FC = () => {
  ...
  const { data: actions, isLoading } = useTeamFindingActions({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">My Team Actions</h1>
        ...
        <div className="flex items-center gap-4">
          {/* View As Dropdown (Admin Only - can view as team or team_manager) */}
          {(role === 'admin' || (isImpersonating && originalUser?.role === 'admin')) && (
            <ViewAsDropdown 
              onSelectUser={handleViewAs}
              filterByRole={['team', 'team_manager']} // Show team and team_manager users only
            />
          )}
        ...
        <Card variant="elevated">
          ...
          <div className="mt-4 min-h-[80px] flex flex-col justify-center">
            {isLoading ? (
              ...
              <>
                <p className="text-4xl font-bold text-yellow-600">{stats.overdue}</p>
                <p className="text-sm text-gray-500 mt-2">
        ...
        <CardHeader 
          ...
          action={
            <Button 
              variant="primary" 
              size="sm"
              onClick={() => exportFindingActions({ auditYear: scorecardFilter, role: 'team' })}
